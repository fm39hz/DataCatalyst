			...
			if (member is INamespaceSymbol nestedNs) {
				...
				if (IsConcept(typeSymbol, conceptAttr, conceptInterface)) {
					...
					if (conceptNsMap != null) {
						var nsName = typeSymbol.ContainingNamespace?.ToDisplayString() ?? "Catalyst.Generated";
						if (!conceptNsMap.ContainsKey(typeSymbol.Name)) {
							conceptNsMap[typeSymbol.Name] = nsName;
						}
					}
				}
			}
	...
	public static void FindAspectsInNamespaceEx(INamespaceSymbol ns, INamedTypeSymbol? aspectAttr, HashSet<string> aspectNames, List<AspectInfo?> results) {
		foreach (var member in ns.GetMembers()) {
			if (member is INamespaceSymbol nestedNs) {
				...
				var nsName = typeSymbol.ContainingNamespace?.ToDisplayString() ?? "Catalyst";

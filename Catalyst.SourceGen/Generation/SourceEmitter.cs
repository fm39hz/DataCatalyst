...
internal sealed class SourceEmitter(
	...
	private readonly Dictionary<string, AspectInfo> _aspectDict = aspectDict;
	private readonly HashSet<string> _aspectNames = aspectNames;
	private readonly HashSet<string> _localAspectNames = localAspectNames;
	private readonly Dictionary<string, string> _conceptNsMap = conceptNsMap;
	private readonly HashSet<string> _conceptNames = conceptNames;
	private readonly Dictionary<string, List<string>> _beingConcepts = beingConcepts;
	private readonly Dictionary<string, HashSet<string>> _conceptAspects = conceptAspects;
	private readonly List<AspectInfo?> _combinedAspects = combinedAspects;
	private readonly Dictionary<string, string> _beingDescriptions = beingDescriptions ?? new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
	...
	public void EmitConceptStructs(
		...
		foreach (var cn in allOntologyConcepts) {
			...
			if (!_conceptNsMap.ContainsKey(cn)) {
				_conceptNsMap[cn] = "Catalyst.Generated";
			}
		}
		writer.EndBlock();
	}
	...
	public void EmitOntologyAspects(
		...
		foreach (var kv in ontologyAspects) {
			...
			if (_aspectDict.ContainsKey(kv.Key)) {
				continue;
			}
	...
	public void EmitDeserializerHelpers(
		...
		foreach (var kv in _conceptAspects) {
			...
			if (!_conceptNames.Contains(kv.Key)) {
				continue;
			}
	...
	public static JsonDataInfo ParseBeingFile(JsonElement root) {
		...
		foreach (var beingProp in root.EnumerateObject()) {
			...
			foreach (var prop in beingObj.EnumerateObject()) {
				...
				nested.Sort();
				fields.Add(new BeingFieldInfo(prop.Name, [.. nested]));
			}
			beings.Add(new BeingEntry(beingKey, [.. fields], [], description));
		}
		return new JsonDataInfo([.. beings]);
	}
}

...
public interface IComponentMaterializer<TTarget> {
	...
	void Materialize(DataEntry entry, TTarget target);
}
...
public sealed class DataMaterializer<TTarget> {
	private readonly List<IComponentMaterializer<TTarget>> _materializers = new();
	...
	public void Register<TComponent>(Action<TTarget, TComponent> apply) where TComponent : struct {
		_materializers.Add(new ComponentMaterializer<TComponent, TTarget>(apply));
	}

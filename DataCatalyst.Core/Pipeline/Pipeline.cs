...
public sealed class Pipeline
{
        ...
        foreach (var src in sorted) {
            ...
            foreach (var kv in result.Mappings)
            {
                if (!ctx.Mappings.TryGetValue(kv.Key, out var list))
                    ctx.Mappings[kv.Key] = list = new List<string>();
    ...
    void PopulateSchemaFromRegistries(PipelineContext ctx)
    {
        foreach (var aspectType in AspectTypeRegistry.RegisteredTypes)
        {
            ...
            _schema.DefineAspect(aspectType.Name, fields);
        }
        ...
        foreach (var conceptType in conceptTypes)
        {
            ...
            {
                if (_schema.GetAspectId(name) < 0)
                {
                    _schema.DefineAspect(name, new Dictionary<string, Type>(StringComparer.OrdinalIgnoreCase));
                }
            }

            _schema.DefineConcept(conceptType.Name, aspectNames);
        }
    }

    static void Merge(PipelineContext ctx) {
    ...
    static void Inherit(PipelineContext ctx) {
        var entries = ctx.RawEntries;

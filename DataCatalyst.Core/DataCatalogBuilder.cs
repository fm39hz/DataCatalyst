...
public static class DataCatalogBuilder {
    public static DataCatalog Resolve(DataGraph graph) {
        ...
        foreach (var entry in ordered) {
            ...
            ResolveEntry(entry, graph, resolved, new HashSet<string>());
        }
    ...
    private static void ResolveEntry(
        ...
        if (resolved.ContainsKey(entry.Key)) return;
    ...
    private static void MergeComponents(DataEntry child, DataEntry parent) {
        foreach (var kv in parent._components) {
            if (!child._components.ContainsKey(kv.Key))
                child._components[kv.Key] = kv.Value;
        }
    }

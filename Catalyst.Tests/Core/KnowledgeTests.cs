...
public class KnowledgeTests {
	private struct K_TestBeingA : IBeing { }
	private struct K_TestConceptFoo : IConcept { }
	private struct K_TestAspectX : IRevealedBy<K_TestConceptFoo> { public int Value; }

	private static Knowledge CreateKnowledge(
		Dictionary<Type, ITypedStoragePool>? pools = null,
		Dictionary<Type, int>? beingIndices = null,
		SchemaRegistry? schema = null) => new Knowledge(
			pools ?? [],
			beingIndices ?? [],
			schema);

...
/// <summary>
///     Orchestrates one UDDSG generation per <see cref="TargetInfo" />. The driver is intentionally lean:
///     <list type="number">
...
///         <item>Run every <see cref="IUddsgPostProcessor" /> against the emitted source.</item>
...
internal static class PipelineDriver {
	public static void Run(SourceProductionContext spc, ImmutableArray<AdditionalText> additionalTexts, TargetInfo target) {
		if (!target.IsPartial) {
			spc.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.TargetNotPartial, target.Location, target.FullyQualifiedName));
			return;
		}

		var matched = FindAdditionalText(additionalTexts, target.JsonPath);
		if (matched is null) {
			spc.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.JsonNotFound, target.Location, target.FullyQualifiedName, target.JsonPath));
			return;
		}

		var rawText = matched.GetText(spc.CancellationToken)?.ToString();
		if (string.IsNullOrWhiteSpace(rawText)) {
			spc.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.JsonInvalid, target.Location, target.FullyQualifiedName, matched.Path, "empty file"));
			return;
		}
		...
		try {
			...
			spc.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.JsonInvalid, target.Location, target.FullyQualifiedName, matched.Path, ex.Message));
			return;
		}
		...
		using (doc) {
			...
			if (entry is null) {
				spc.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.EntryPointMissing, target.Location, target.FullyQualifiedName, target.EntryPoint, matched.Path));
				return;
			}

			var ctx = new UddsgGenerationContext(
			...
			foreach (var candidate in UddsgPluginRegistry.Readers) {
			...
			if (reader is null) {
				spc.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.NoReaderMatched, target.Location, target.FullyQualifiedName, target.EntryPoint, entry.Value.ValueKind));
				return;
			}
			...
			foreach (var candidate in UddsgPluginRegistry.SchemaProviders) {
			...
			if (schemaProvider is null) {
				spc.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.NoSchemaProviderMatched, target.Location, target.FullyQualifiedName));
				return;
			}
			...
			foreach (var candidate in UddsgPluginRegistry.Emitters) {
				if (candidate.Applies(ctx)) {
					emitter = candidate;
					break;
				}
			...
			if (emitter is null) {
				spc.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.NoEmitterMatched, target.Location, target.FullyQualifiedName));
				return;
			}
			...
			foreach (var post in UddsgPluginRegistry.PostProcessors) {
				post.After(src, ctx);
			}

...
/// <summary>
///     Static registry of every UDDSG plugin loaded into the current AppDomain. Plugins self-register
...
///     </para>
/// </summary>
public static class UddsgPluginRegistry {
	...
	private static readonly List<PluginEntry<IUddsgPostProcessor>> _postProcessors = [];
	...
	public static void Register(IUddsgPostProcessor plugin, params Type[] dependsOn) => _postProcessors.Add(CreateEntry(plugin, dependsOn));
	...
	public static IReadOnlyList<IUddsgPostProcessor> PostProcessors => Order(_postProcessors);
	...
	private static IReadOnlyList<TPlugin> Order<TPlugin>(List<PluginEntry<TPlugin>> entries) {
		...
		if (ordered.Count != map.Count) {
			...
			throw new InvalidOperationException("UDDSG plugin dependency cycle detected: " + cycle);
		}

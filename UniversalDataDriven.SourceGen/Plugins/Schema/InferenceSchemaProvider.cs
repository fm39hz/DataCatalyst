namespace UniversalDataDriven.Plugins.Schema;
...
///         provider emits <see cref="UddsgDiagnostics.InferenceUnsupported" /> and aborts.
///     </para>
/// </summary>
[UddsgPlugin(typeof(ISchemaProvider))]
internal sealed class InferenceSchemaProvider : ISchemaProvider {
	[ModuleInitializer]
	internal static void Register() => UddsgPluginRegistry.Register(new InferenceSchemaProvider());

	public string Name => "Inference";

	public bool Applies(UddsgGenerationContext ctx) => ctx.Template is null;

	public SchemaInfo? Build(IReadOnlyList<RowData> rows, UddsgGenerationContext ctx) {
		if (rows.Count == 0) {
	...
	private SchemaType? InferTypeFromValue(JsonValueModel v, UddsgGenerationContext ctx, string path) {
		switch (v.Kind) {
			...
			case JsonValueKind.Array: {
				if (v.ArrayItems!.Count == 0) {
					ctx.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.InferenceUnsupported, ctx.Location, ctx.TargetFullyQualifiedName, path, "empty array — element type cannot be inferred"));
					return null;
				}
			...
			case JsonValueKind.Null:
				ctx.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.InferenceUnsupported, ctx.Location, ctx.TargetFullyQualifiedName, path, "null leading value — type cannot be inferred"));
				return null;
			default:
				ctx.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.InferenceUnsupported, ctx.Location, ctx.TargetFullyQualifiedName, path, $"unsupported JSON kind {v.Kind}"));
				return null;
		}
	}

	private static SchemaType? InferPrimitive(JsonValueModel v, UddsgGenerationContext ctx, string path) {
		...
		ctx.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.InferenceUnsupported, ctx.Location, ctx.TargetFullyQualifiedName, path, $"no IPrimitiveTypeRule accepts JSON kind {v.Kind}"));
		return null;
	}

	private SchemaType? WidenType(SchemaType existing, JsonValueModel v, UddsgGenerationContext ctx, string path) {
	...
	private SchemaType? WidenSchemaType(SchemaType a, SchemaType b, UddsgGenerationContext ctx, string path) {
		if (a.Equals(b)) {
			return a;
		}
		...
		if (a.IsPrimitive && b.IsPrimitive) {
			...
			if (widened is null) {
				ctx.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.InferenceUnsupported, ctx.Location, ctx.TargetFullyQualifiedName, path, $"incompatible primitives: {a.Primitive} vs {b.Primitive}"));
				return null;
			}
		...
		ctx.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.InferenceUnsupported, ctx.Location, ctx.TargetFullyQualifiedName, path, "rows disagree on column shape"));
		return null;
	}

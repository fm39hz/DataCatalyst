...
///     no template member raise <see cref="UddsgDiagnostics.TemplateMissingMember" /> as a warning so users
...
internal sealed class TemplateSchemaProvider : ISchemaProvider {
	[ModuleInitializer]
	internal static void Register() => UddsgPluginRegistry.Register(new TemplateSchemaProvider());

	public string Name => "Template";

	public bool Applies(UddsgGenerationContext ctx) => ctx.Template is not null;
		...
		foreach (var row in rows) {
			foreach (var kv in row.Values) {
				if (!byName.ContainsKey(kv.Key)) {
					ctx.ReportDiagnostic(Diagnostic.Create(UddsgDiagnostics.TemplateMissingMember, ctx.Location, ctx.TargetFullyQualifiedName, template.FullyQualifiedName, kv.Key));
				}
			}
		}

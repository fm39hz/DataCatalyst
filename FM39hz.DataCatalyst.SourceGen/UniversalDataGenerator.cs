...
public sealed class UniversalDataGenerator : IIncrementalGenerator {
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        ...
        context.RegisterPostInitializationOutput(static ctx => {
            ...
            ctx.AddSource("ModPluginAttribute.g.cs", SourceText.From("""
                using System;
        ...
        // Collect [DataDsl] — assembly level
        var dsls = context.CompilationProvider
            .Select(static (comp, _) => ExtractDataDsls(comp));

        context.RegisterSourceOutput(dsls, static (spc, dslList) => {
            if (dslList.IsDefaultOrEmpty) return;
            var sb = new StringBuilder();
            sb.Append("// <auto-generated/>\n#nullable enable\n\nnamespace FM39hz.DataCatalyst.Runtime {\n\tpublic static partial class DataDslRegistrations {\n");
            sb.Append("\t\t[System.Runtime.CompilerServices.ModuleInitializer]\n\t\tinternal static void Register() {\n");
            foreach (var (id, typeFqn) in dslList) {
                sb.Append("\t\t\tglobal::FM39hz.DataCatalyst.Runtime.DataDslRegistry.Register(\"")
                  .Append(id).Append("\", typeof(").Append(typeFqn).AppendLine("));");
            }
            sb.Append("\t\t}\n\t}\n}");
            spc.AddSource("DataDslRegistrations.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
        });
    ...
    private static ImmutableArray<(string Id, string TypeFqn)> ExtractDataDsls(Compilation compilation) {
        var builder = ImmutableArray.CreateBuilder<(string, string)>();
        foreach (var attr in compilation.Assembly.GetAttributes()) {
            if (attr.AttributeClass?.Name != "DataDslAttribute") continue;
            if (attr.ConstructorArguments.Length < 2) continue;
            var id = attr.ConstructorArguments[0].Value as string;
            var typeSymbol = attr.ConstructorArguments[1].Value as INamedTypeSymbol;
            if (id is null || typeSymbol is null) continue;
            builder.Add((id, typeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)));
        }

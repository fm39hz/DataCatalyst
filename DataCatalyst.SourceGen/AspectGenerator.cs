...
public sealed class AspectPropertyInfo : IEquatable<AspectPropertyInfo>
{
    public string Name { get; }
    public string Type { get; }

    public AspectPropertyInfo(string name, string type)
    {
        Name = name;
        Type = type;
    }
...
public sealed class AspectInfo : IEquatable<AspectInfo>
{
    ...
    public string PropertiesString { get; } // e.g. "Current:int;Max:int"

    public AspectInfo(string name, string ns, string propertiesString)
    {
        Name = name;
        Namespace = ns;
...
public sealed class ConceptInfo : IEquatable<ConceptInfo>
{
    public string Name { get; }
    public string Namespace { get; }

    public ConceptInfo(string name, string ns)
    {
        Name = name;
        Namespace = ns;
    }
...
public sealed class JsonDataInfo : IEquatable<JsonDataInfo>
{
    public string EntriesString { get; }

    public JsonDataInfo(string entriesString)
    {
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var jsonData = context.AdditionalTextsProvider
            ...
            .Select((t, _) => {
                ...
                try
                {
                    using var d = System.Text.Json.JsonDocument.Parse(content);
                    var r = d.RootElement;
                    if (r.ValueKind != System.Text.Json.JsonValueKind.Object) return "";
                    ...
                    foreach (var entryProp in r.EnumerateObject())
                    {
                        ...
                        if (entryObj.ValueKind != JsonValueKind.Object) continue;
            ...
            .Collect()
            .Select((ImmutableArray<string> ar, System.Threading.CancellationToken _) => {
                var merged = new Dictionary<string, Dictionary<string, HashSet<string>>>(StringComparer.OrdinalIgnoreCase);
                foreach (string fileContent in ar)
                {
                    foreach (var entryPart in fileContent.Split('|'))
                    {
                        ...
                        {
                            if (string.IsNullOrEmpty(propPart)) continue;
                            ...
                            foreach (var n in nestedStr.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                                hs.Add(n);
                        }
                    }
                }
        ...
        context.RegisterSourceOutput(combined, (spc, input) => {
            ...
            if (aspects.Length == 0 && concepts.Length == 0) return;
            ...
            if (!string.IsNullOrEmpty(jsonData.EntriesString))
            {
                ...
                {
                    if (string.IsNullOrEmpty(entryPart)) continue;
                    var bracketIdx = entryPart.IndexOf('[');
                    if (bracketIdx <= 0) continue;
                    var entryKey = entryPart.Substring(0, bracketIdx);
                    var body = entryPart.Substring(bracketIdx + 1, entryPart.Length - bracketIdx - 2);
                    ...
                    if (!entryConcepts.TryGetValue(entryKey, out var cList))
                        entryConcepts[entryKey] = cList = new List<string>();
                    ...
                    foreach (var propPart in body.Split(';'))
                    {
                        ...
                        if (propName.StartsWith("$"))
                        {
                            ...
                            if (conceptNames.Contains(conceptName))
                            {
                                ...
                                foreach (var a in nestedStr.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                                    aSet.Add(a);
                            }
                            else
                            {
            ...
            foreach (var asp in aspects)
            {
                ...
                foreach (var part in propParts)
                {
                    ...
                    bool isNullable = pType.EndsWith("?");
                    bool isValueType = pType.EndsWith("int") || pType.EndsWith("System.Int32") ||
...
public sealed class {cn}Pool : global::DataCatalyst.Storage.IStoragePool {{
    ...
    static AspectInfo? GetAspectInfo(GeneratorAttributeSyntaxContext context, System.Threading.CancellationToken token)
    {
    ...
    static ConceptInfo? GetConceptInfo(GeneratorAttributeSyntaxContext context, System.Threading.CancellationToken token)
    {
        var symbol = context.TargetSymbol as INamedTypeSymbol;
        if (symbol == null) return null;
    ...
    static string Cast(string type, string varName) {
        if (type.EndsWith("int") || type.EndsWith("System.Int32")) return $"global::System.Convert.ToInt32({varName})";
        if (type.EndsWith("long") || type.EndsWith("System.Int64")) return $"global::System.Convert.ToInt64({varName})";
        if (type.EndsWith("float") || type.EndsWith("System.Single")) return $"global::System.Convert.ToSingle({varName})";
        if (type.EndsWith("double") || type.EndsWith("System.Double")) return $"global::System.Convert.ToDouble({varName})";
        if (type.EndsWith("bool") || type.EndsWith("System.Boolean")) return $"global::System.Convert.ToBoolean({varName})";

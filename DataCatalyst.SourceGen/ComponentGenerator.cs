...
public sealed class ComponentGenerator : IIncrementalGenerator {
	...
	private static void EmitMerge(SourceProductionContext spc,
		...
		sb.AppendLine("namespace DataCatalyst.Core {");
		sb.AppendLine("  using System;");
		sb.AppendLine("  using DataCatalyst.Core;");
		sb.AppendLine("  internal static class ComponentMergeRegistrations {");
		sb.AppendLine("    [System.Runtime.CompilerServices.ModuleInitializer]");
		sb.AppendLine("    internal static void Init() {");

			foreach (var p in valid) {
				var shortName = GetShortName(p.FullType!);
				sb.AppendLine($"      ComponentMerger.Register<{p.FullType}>((parent, child) => {{");
				sb.AppendLine($"        var p = ({p.FullType})parent;");
				sb.AppendLine($"        var c = ({p.FullType})child;");
				foreach (var field in p.Fields!) {
					// Format: "Name:global::System.Single"
					var colonIdx = field.LastIndexOf(':');
					if (colonIdx < 0) continue;
					var fieldName = field.Substring(0, colonIdx);
					var fieldType = field.Substring(colonIdx + 1);
					sb.AppendLine($"        if (!global::System.Collections.Generic.EqualityComparer<{fieldType}>.Default.Equals(c.{fieldName}, default)) p.{fieldName} = c.{fieldName};");
				}
